/**
 * AI Step Driver
 * Calls the target AI agent endpoint, extracts response text,
 * computes semantic similarity via Claude, compares against threshold.
 */
import Anthropic from '@anthropic-ai/sdk'
import type { StepDef, StepResult, CtxMap, RunContext } from '../types'
import { interpolate } from '../interpolate'
import { AiStepConfig } from '../../dag'

// Lazy singleton — avoids init errors when ANTHROPIC_API_KEY is absent in build
...
export async function runAiStep(
  ...
  // Call the target agent (if agent_url specified) or use prompt directly for eval
  if (config.agent_url) {
    try {
      ...
      // Extract response text — common LLM response patterns
      const path = config.response_path
      if (path) {
        agentResponse = String(extractNested(json, path) ?? '')
      } else {
        const choices = json['choices']
        const firstChoice = Array.isArray(choices) ? choices[0] : undefined
        const msg = firstChoice && typeof firstChoice === 'object' ? (firstChoice as Record<string,unknown>)['message'] : undefined
        const content = msg && typeof msg === 'object' ? (msg as Record<string,unknown>)['content'] : undefined
        agentResponse = String(
          content ?? json['response'] ?? json['content'] ?? json['text'] ?? JSON.stringify(json)
        )
      }
    } catch (e) {
    ...
    // No agent URL — evaluate prompt directly as the "response" (useful for testing Claude itself)
  ...
  if (fetchError) {
    ...
    return {
      ...
      ctxOutputs: {}, responseBody: null,
      responseMeta: {}, assertions: [], errorMessage: fetchError, artifacts: [],
    }
  }
  ...
  return {
    ...
    assertions: [{
      name: `semantic_similarity >= ${config.threshold}`,
      passed,
...
async function computeSemanticScore(expected: string, actual: string): Promise<number> {
  ...
  const msg = await client.messages.create({
    ...
    messages: [{
      role: 'user',
      content: `Rate semantic similarity 0.00-1.00 (two decimal places, no other text):
Expected: ${expected}
Actual: ${actual}`,
    }],
  })
